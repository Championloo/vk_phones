...
groupid = input('Введите ID или короткое имя (из ссылки) группы/паблика: ')
...
with con:
	cursor.execute("""CREATE TABLE if not exists vk (id int, name text, last text, phone text)""")
...
with con:
	...
	for i in tqdm(range(len(data)), smoothing=0, miniters=None):
		try:
			user = vk_api.users.get(user_ids=str(data[i][0]), v=5.92, fields='contacts', lang='ru')[0]
			phone = ''
			if 'is_closed' in user and user['is_closed']==False:
				...
				if 'mobile_phone' in user and len(user['mobile_phone'])>8:
					...
					if len(truephone)>9:
						phone += truephone+'; '
			cursor.execute("UPDATE vk SET name=?, last=?, phone=? where id=?",(user['first_name'], user['last_name'], phone, str(data[i][0])))
			con.commit()
			time.sleep(0.001)
		except:
			time.sleep(15)
...
ws.write(0, 2, 'Ссылка на страницу', style)
